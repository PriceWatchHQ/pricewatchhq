/**
 * seed-week2.js — Seeds week 2 X posts (2-3/day) into scheduled_posts table.
 * Run once: node src/seed-week2.js
 */
...
const posts = [
  ...
  { text: `Most price monitoring tools cost $500+/mo and are built for enterprise.\n\nWe built PriceWatch HQ for the small/medium store owner who just wants to know when a competitor changes their price.\n\nStarts at $0. → https://pricewatchhq.com`, hour: 14, day: 29 },
...
const existing = db.prepare('SELECT COUNT(*) as count FROM scheduled_posts WHERE posted = 0').get();
console.log(`Existing unposted posts: ${existing.count}`);

const insert = db.prepare('INSERT INTO scheduled_posts (text, scheduled_for, posted, created_at) VALUES (?, ?, 0, ?)');

let added = 0;
for (const post of posts) {
  const date = new Date(2026, 2, post.day, post.hour, 0, 0); // March is month 2 (0-indexed)
  insert.run(post.text, date.getTime(), Date.now());
  console.log(`Scheduled: March ${post.day} @ ${post.hour}:00`);
  added++;
}
